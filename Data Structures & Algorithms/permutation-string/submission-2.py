# Time complexity: O(n)
# Space complexity: O(1)  - Because even if the input size grows the max, space is 
# still 26 key hashmap
...
class Solution:
    def checkInclusion(self, s1: str, s2: str) -> bool:
        ...
        left = 0 
        right = len(s1)
        dict_s1 = dict(Counter(s1)) #O(m)
        dict_s2 = dict(Counter(s2[left:right])) #O(m)
